...
옵션:
  -b, --base <branch>   비교 기준 브랜치 (기본: origin 기본 브랜치 자동 감지)
      --model <name>    Gemini 모델 (기본: gemini-2.5-flash)
...
async function generateBody({ apiKey, model, log, diff }) {
...
${diff}`;
  ...
  if (!res.ok) {
    ...
    fail(`Gemini 호출 실패(${res.status}): ${text.slice(0, 300)}`);
  }
  const data = await res.json();
  const body = data?.candidates?.[0]?.content?.parts?.[0]?.text;
  if (!body || !body.trim()) fail("AI 응답이 비어 있습니다.");
  return body.trim();
}
...
async function main() {
  ...
  const model = opts.model || process.env.AI_PR_MODEL || "gemini-2.5-flash";

  console.error("AI가 PR 본문 생성 중...");
  const body = await generateBody({ apiKey, model, log, diff });
